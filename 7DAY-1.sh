...
cpu_utilization() {
    echo "CPU Utilization (User + System) for Past 7 Days:"
    echo "------------------------------------------------" >> $OUTPUT_FILE
...
memory_utilization() {
    echo "Memory Utilization for Past 7 Days:"
    echo "------------------------------------------------" >> $OUTPUT_FILE
    ...
    for i in {1..7}; do
        ...
        if [[ -f $FILE ]]; then
            MEM_VALUES=$(sar -r -f $FILE | awk 'NR>3 {print $4}') # %memory used
...
disk_utilization() {
    echo "Disk Utilization for Past 7 Days:"
    echo "------------------------------------------------" >> $OUTPUT_FILE
